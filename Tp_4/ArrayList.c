...
ArrayList* al_clone(ArrayList* pList)
{
    ArrayList* returnAux = NULL;
    if(pList != NULL)
        returnAux=al_subList(pList,0,pList->size-1);
    return returnAux;
}
...
int al_push(ArrayList* pList, int index, void* pElement)
{
    ...
    if(pList != NULL && pElement != NULL && index >=0 && index <= (pList->size))
    {
        ...
        {
            if(!al_set(pList,index,pElement))
            {
                returnAux=0;
                pList->size++;
            }
...
ArrayList* al_subList(ArrayList* pList,int from,int to)
{
    void* returnAux = NULL;
    int i;
    if(pList != NULL && from >=0 && to >=0 && to<= pList->size && from<=to )
    {
...
int al_containsAll(ArrayList* pList,ArrayList* pList2)
{
    ...
    if(pList != NULL && pList2 != NULL)
    {
          ...
          for(i=0;i < pList->size;i++)
          {
               if(al_contains(pList,al_get(pList,i)) == 1)
                  counter++;
                else
                    break;
          }
          if(counter == pList2->size)
            returnAux=1;
    }
    return returnAux;
}
    ...
    if(pList != NULL && pFunc!= NULL && (order == 0 || order == 1))
    {
        for(i=0;i<pList->size-1;i++)
        {
            for(j=i+1;j<pList->size;j++)
            {
                ...
                if(pFunc(al_get(pList,j),al_get(pList,i)) == 1 && order == 0)
                {
                    pAux=al_get(pList,j);
                    al_set(pList,j,al_get(pList,i));
                    al_set(pList,i,pAux);
                }
                else if(pFunc(al_get(pList,j),al_get(pList,i)) == -1 && order ==1)
                {
                    pAux=al_get(pList,i);
...
int contract(ArrayList* pList,int index)
{
    ...
    if(pList != NULL && index>=0 )
    {
        for(i=index+1;i<=pList->size;i++)
            al_set(pList,i-1,al_get(pList,i));
        pList->pElements=realloc(pList->pElements,sizeof(void*)*(pList->reservedSize-1));
        pList->size--;
        returnAux=0;
    }
    return returnAux;
